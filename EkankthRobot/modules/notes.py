...
import EkankthRobot.modules.sql.notes_sql as sql
from EkankthRobot import DRAGONS, JOIN_LOGGER, LOGGER, SUPPORT_CHAT, dispatcher
from EkankthRobot.modules.disable import DisableAbleCommandHandler
from EkankthRobot.modules.helper_funcs.chat_status import connection_status, user_admin
from EkankthRobot.modules.helper_funcs.misc import build_keyboard, revert_buttons
from EkankthRobot.modules.helper_funcs.msg_types import get_note_type
    ...
    if member.status != "creator" and user.id not in DRAGONS:
        ...
        buttons = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text="🕊️Delete all notes🕊️", callback_data="notes_rmall"
                    )
                ],
                [InlineKeyboardButton(text="☀️Cancel☀️", callback_data="notes_cancel")],
            ]
        )
        update.effective_message.reply_text(
